...
export const getPublicTravelStories = async (req, res, next) => {
  try {
    ...
    // Filter out stories from the current user
    const filteredStories = travelStories.filter(story => story.userId._id.toString() !== userId)
...
export const likeTravelStory = async (req, res, next) => {
  const { id } = req.params
  const userId = req.user.id
  ...
  try {
    ...
    const isLiked = travelStory.likes.includes(userId)

    if (isLiked) {
      // Unlike the story
      travelStory.likes = travelStory.likes.filter(id => id.toString() !== userId)
    } else {
      // Like the story
